...
struct Isoform
{
  ...
  Isoform(const string &mature_id, const string&mature_seq, const string &read_seq,
          int read_num, int dist)
...
int find_seed_in_seq(const string &seed, const string &seq)
{
                   ...
                   int max_ed_5p, int max_ed_3p)
{
  ...
  else if (max_ed_5p != -1 and max_ed_3p != -1)
  {
    if (lev_5p > max_ed_5p)
      dist = -5;
    if (lev_3p > max_ed_3p)
                                 ...
                                 int max_ed_5p, int max_ed_3p)
{
  ...
  for (Mirna *mirna : mirnas)
  {
    ...
    int read_seq_index_5p = find_seed_in_seq(seed_seq, read_seq);
    if (read_seq_index_5p == -1)
      continue;

    int mature_index_5p = mature_seq.find(seed_seq);
    if (mature_index_5p == string::npos)
    {
      stop("Motif: %s is not found in consensus", seed_seq);
    }

    int mature_index_3p = mature_index_5p + seed_seq.size();
    int read_seq_index_3p = read_seq_index_5p + seed_seq.size();
