...
#' Calculate Tissue Specificity Index (TSI) for a Single Feature
#'
...
#'
#' @param exp A numeric data frame representing non-negative expression values
...
calc_one_tsi <- function(exp){
  if (any(exp < 0)) stop("Expression values must be non-negative")
  if (all(exp == 0)) stop("At least one value must be non-negative")
  if ((length(exp) - 1) == 0) stop("length(exp) - 1 must be positive")

  max_exp <- max(exp)
  sum(1 - exp/max_exp) / (length(exp) - 1)
}
